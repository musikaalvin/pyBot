...
// Set global variables from config at startup
global.ghostMode = config.ghostMode;
global.fakeOnline = config.fakeOnline;

// Fake Online: Keep appearing online even when not actively using bot
setInterval(async () => {
  if (global.fakeOnline) {
    try {
...
async function startBot() {
  ...
  // Initialize auto recording interval
  const { startAutoRecording } = require('./handler');
  startAutoRecording(sock);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'open') {
      ...
      clearInterval(watchdogInterval);
      // Stop auto recording interval when connection closes
      const { stopAutoRecording } = require('./handler');
      stopAutoRecording();
    }
  });
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Track bot startup/log events globally
global.botStats = global.botStats || {
  startTime: Date.now(),
  restarts: (global.botStats?.restarts || 0) + 1,
  lastSeen: [],
  totalUsers: new Set(),
  commandCount: 0
};

// Count commands on startup
if (global.botStats.commandCount === 0) {
  try {
    const fs = require('fs');
    const path = require('path');
    ...
    if (fs.existsSync(commandsPath)) {
      ...
      categories.forEach(cat => {
        ...
        if (fs.statSync(catPath).isDirectory()) {
          ...
          global.botStats.commandCount += files.length;
        }
      });
    }
  } catch (e) {}
...
// Send welcome message to owner
      try {
        ...
        // Get formatted time (Uganda/Kampala timezone, 12-hour format)
        const now = new Date();
        const timeString = now.toLocaleTimeString('en-UG', { timeZone: 'Africa/Kampala', hour: 'numeric', minute: '2-digit' });
        const dateString = now.toLocaleDateString('en-UG', { timeZone: 'Africa/Kampala', day: 'numeric', month: 'short', year: 'numeric' });
        
        if (fs.existsSync(imgPath)) {
          await sock.sendMessage(ownerJid, {
            image: fs.readFileSync(imgPath),
            caption: `╭━━━━━━━━━━━━━━━⊷\n┃ ⚡ *${config.botName}* ONLINE\n╰━━━━━━━━━━━━━━━⊷\n\n👋 *Welcome back, ${config.ownerName[0]}!*\n\n🤖 *Bot Status:* Active\n⏰ *Time:* ${dateString} ${timeString}\n🔄 *Restart:* #${global.botStats.restarts}\n\n📌 *Quick Info:*\n• Prefix: ${config.prefix}\n• Mode: ${config.selfMode ? 'Private' : 'Public'}\n• ${global.botStats.commandCount}+ Commands\n\n💡 *Type ${config.prefix}menu for all commands*\n\n_Powered by ${config.author}_`
          });
        } else {
          await sock.sendMessage(ownerJid, {
            text: `╭━━━━━━━━━━━━━━━⊷\n┃ ⚡ *${config.botName}* ONLINE\n╰━━━━━━━━━━━━━━━⊷\n\n👋 *Welcome back, ${config.ownerName[0]}!*\n\n🤖 *Bot Status:* Active\n⏰ *Time:* ${dateString} ${timeString}\n🔄 *Restart:* #${global.botStats.restarts}\n\n📌 *Quick Info:*\n• Prefix: ${config.prefix}\n• Mode: ${config.selfMode ? 'Private' : 'Public'}\n• ${global.botStats.commandCount}+ Commands\n\n💡 *Type ${config.prefix}menu for all commands*\n\n_Powered by ${config.author}_`
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      setImmediate(async () => {
        // Ghost Mode: Don't mark messages as read (no blue ticks)
        try {
          if (!msg.key.fromMe) {
            // Only skip marking read when ghost mode is on, for messages not from bot
            if (global.ghostMode === true) {
              // Just return, don't mark as read
              return;
            }
            // Otherwise mark as read normally  
...
// Spy Mode - forward messages from spy targets
        try {
          // Only run spy mode if enabled in config AND not from bot
          if (config.spyMode !== true || msg.key.fromMe) return;
          ...
          let shouldSpy = false;
          if (isGroup) {
            shouldSpy = spy.groups.includes(jid);
          } else {
            // Check if sender or jid is in DM spy list
            shouldSpy = spy.dms.includes(sender) || spy.dms.includes(jid);
          }
           
          if (shouldSpy) {
            console.log('[Spy] Forwarding message from:', senderNumber);
            try {
              const owner = config.ownerNumber[0];
              const txt = getMessageText(msg);
              const fromName = msg.pushName || sender.split('@')[0];
              ...
              let msg2 = `🕵️ ${isGroup ? 'GROUP' : 'DM'} SPY\n`;
              msg2 += `━━━━━━━━━━━━━━━━\n`;
              msg2 += `👤 Sender: ${fromName}\n`;
              msg2 += `📱 Number: ${fromNum}\n`;
              msg2 += `⏰ Time: ${timeStr}\n`;
              msg2 += `━━━━━━━━━━━━━━━━\n`;
              msg2 += `📩 ${txt || '[Media/Image/Video]'}`;
              
              await sock.sendMessage(owner + '@s.whatsapp.net', { text: msg2 });
            } catch (e) {
              console.log('[Spy] Error:', e.message);
            }
          }
        } catch (e) {
          console.log('[Spy] Setup error:', e.message);
        }
});
    }
  });
  ...
  // Message updates (silently handled)
  ...
  // Message delete events (for antidelete feature)
...
// Start the bot
const botLogo = '⚡'; // Lightning bolt icon
console.log(`${botLogo} ${config.botName} - ${config.botDescription || 'Advanced WhatsApp MD Bot'}\n`);
console.log(`📦 Bot Name: ${config.botName}`);
console.log(`📝 Description: ${config.botDescription || 'Advanced WhatsApp MD Bot'}`);
console.log(`⚡ Prefix: ${config.prefix}`);
const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(',') : config.ownerName;
console.log(`👑 Owner: ${ownerNames}\n`);
